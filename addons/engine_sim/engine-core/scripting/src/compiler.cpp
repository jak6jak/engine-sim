...
void es_script::Compiler::initialize() {
    ...
    // Search paths relative to the script location (in assets/)
    // The script file's parent directory will be used as the base
    m_compiler->addSearchPath("./");      // Same directory as the script
    m_compiler->addSearchPath("../");     // One level up (for files in subdirectories)
    m_compiler->addSearchPath("../../");  // Two levels up (for deeply nested files)
    m_compiler->addSearchPath("../../../"); // Three levels up
    m_compiler->addSearchPath("../../es/");  // Original paths for compatibility
